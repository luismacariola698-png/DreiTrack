...
class StorageLocation(Base):
    ...
    __table_args__ = (UniqueConstraint('organization_id', 'code', name='uq_storage_location_org_code'),)
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    organization_id: Mapped[int] = mapped_column(ForeignKey('organizations.id'), nullable=False, index=True)
...
class AssetType(Base):
    ...
    __table_args__ = (UniqueConstraint('organization_id', 'name', name='uq_asset_type_org_name'),)
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    organization_id: Mapped[int] = mapped_column(ForeignKey('organizations.id'), nullable=False, index=True)
...
class Item(Base):
    ...
    default_storage_location_id: Mapped[int | None] = mapped_column(ForeignKey('storage_locations.id'), nullable=True)
...
class Asset(Base):
    ...
    __table_args__ = (UniqueConstraint('organization_id', 'code', name='uq_asset_org_code'), UniqueConstraint('organization_id', 'serial_number', name='uq_asset_org_serial'))
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    organization_id: Mapped[int] = mapped_column(ForeignKey('organizations.id'), nullable=False, index=True)
    code: Mapped[str] = mapped_column(String(120), nullable=False)
    name: Mapped[str | None] = mapped_column(String(160), nullable=True)
    asset_type_id: Mapped[int | None] = mapped_column(ForeignKey('asset_types.id'), nullable=True)
...
class AssetComponent(Base):
    ...
    __table_args__ = (UniqueConstraint('organization_id', 'asset_id', 'item_id', name='uq_asset_component_org_asset_item'),)
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    organization_id: Mapped[int] = mapped_column(ForeignKey('organizations.id'), nullable=False, index=True)
...
class PurchaseOrder(Base):
    ...
    __table_args__ = (UniqueConstraint('organization_id', 'po_number', name='uq_purchase_order_org_number'),)
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    organization_id: Mapped[int] = mapped_column(ForeignKey('organizations.id'), nullable=False, index=True)
...
class PurchaseOrderLine(Base):
    ...
    unit_cost: Mapped[float | None] = mapped_column(Float, nullable=True)
TENANT_SCOPED_MODELS = (Site, StorageLocation, AppUser, AssetType, Item, Asset, Transaction, AssetComponent, StockRequest, PurchaseOrder, PurchaseOrderLine)
...
def _scope_queries_to_organization(execute_state) -> None:
    ...
    for model in TENANT_SCOPED_MODELS:
        statement = statement.options(with_loader_criteria(model, model.organization_id == organization_id, include_aliases=True))
