    ...
    try:
        ...
        if organization is None:
            organization = Organization(
                ...
                slug=unique_organization_slug(db, "Demo Organization"),
                is_active=True,
            )
            db.add(organization)
            db.flush()
        ...
        location = db.scalar(
            select(StorageLocation).order_by(StorageLocation.id)
        )
        asset_type = db.scalar(
            select(AssetType).where(AssetType.name == "Equipment")
        ) or db.scalar(select(AssetType).order_by(AssetType.id))
        ...
        if asset is None:
            asset = Asset(
                ...
                serial_number="DEMO-A001",
                location="Workshop A",
                status="ACTIVE",
                notes="Generic demonstration asset.",
            )
            db.add(asset)
            db.commit()
            db.refresh(asset)
        ...
        component = db.scalar(
            select(AssetComponent).where(
                ...
                AssetComponent.item_id == item.id,
            )
        )
