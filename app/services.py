...
def reserved_for_item(
    db: Session,
    item_id: int,
    storage_location_id: int | None = None,
) -> int:
    query = select(StockRequest).where(
        StockRequest.item_id == item_id,
...
def available_for_item(
    db: Session,
    item_id: int,
    storage_location_id: int | None = None,
) -> int:
    return stock_for_item(db, item_id, storage_location_id) - reserved_for_item(
        db, item_id, storage_location_id
    )
...
def create_stock_transfer(
    ...
    outgoing = Transaction(
        ...
        item_id=item.id,
        storage_location_id=from_location.id,
        transaction_type="OUT",
        quantity=quantity,
        reason=f"Transfer to {to_location.code}",
        actor=actor,
    ...
    incoming = Transaction(
        organization_id=organization_id,
        item_id=item.id,
        storage_location_id=to_location.id,
        transaction_type="IN",
        quantity=quantity,
        reason=f"Transfer from {from_location.code}",
        actor=actor,
        reference=transfer_reference,
        notes=notes,
    )
    ...
    if request.asset_id is not None:
        ...
        component = db.scalar(
            select(AssetComponent).where(
                AssetComponent.asset_id == request.asset_id,
                AssetComponent.item_id == request.item_id,
            )
        )
        ...
        if component is not None:
            ...
            if request.quantity > remaining_required:
                raise ValueError(
                    "Requested quantity exceeds the asset's remaining tracked requirement."
                )
...
def create_purchase_order(
    ...
    existing_po = db.scalar(
        select(PurchaseOrder).where(PurchaseOrder.po_number == po_number.strip())
    )
...
def receive_purchase_order(
    ...
    all_received = all(
        line.quantity_received >= line.quantity_ordered for line in order_lines
    )
...
def return_component_from_asset(
    db: Session,
    asset_component_id: int,
    quantity: int,
    ...
    transaction = Transaction(
        ...
        transaction_type="IN",
        quantity=quantity,
        reason="Returned from asset",
        actor=returned_by,
        reference=f"ASSET-{component.asset_id}",
        notes=notes,
    )
    db.add(transaction)
    component.quantity_assigned -= quantity
