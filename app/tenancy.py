...
DEFAULT_ASSET_TYPES = ('Equipment', 'Machine', 'Robot', 'Vehicle', '3D Printer', 'Production Line', 'Test Rig', 'Laboratory Equipment', 'Electrical Equipment', 'Tooling')

def slugify(value: str) -> str:
    slug = re.sub('[^a-z0-9]+', '-', value.strip().lower()).strip('-')
...
def create_organization_defaults(db: Session, organization: Organization) -> tuple[Site, StorageLocation]:
    site = Site(organization_id=organization.id, code='MAIN', name='Main Site', is_active=True)
    db.add(site)
    db.flush()
    location = StorageLocation(organization_id=organization.id, site_id=site.id, code='MAIN-STORES', name='Main Stores', location_type='STORAGE', is_active=True)
    db.add(location)
    for name in DEFAULT_ASSET_TYPES:
        db.add(AssetType(organization_id=organization.id, name=name, is_active=True))
    db.flush()
