...
from app.ai.agent import (
    DreiAgentInputError,
    DreiAgentItemNotFoundError,
    ask_drei_about_attention,
    ask_drei_about_item,
    ask_drei_about_overdue_orders,
)
...
async def authentication_context(request: Request, call_next):
    ...
    with SessionLocal() as db:
        ...
        # A company record without any user is an incomplete installation.
        # Do not expose operational pages; an administrator must repair the
        # database rather than creating a second company.
        if user_count == 0 and organization_count > 0:
            if path == "/login":
                return await call_next(request)
        ...
        # Store detached, template-safe snapshots instead of ORM objects.
        ...
        request.state.organization = SimpleNamespace(
            id=organization.id,
...
# SessionMiddleware must wrap the authentication middleware so request.session
# is available inside authentication_context.
app.add_middleware(
    SessionMiddleware,
    secret_key=session_secret(),
    same_site="lax",
    https_only=os.getenv("DREITRACK_HTTPS_ONLY", "0").strip().lower() in {"1", "true", "yes", "on"},
    max_age=60 * 60 * 12,
)
...
def on_order_for_item(db: Session, item_id: int) -> int:
    lines = db.scalars(
        select(PurchaseOrderLine).where(PurchaseOrderLine.item_id == item_id)
    ).all()
    return sum(
        max(line.quantity_ordered - line.quantity_received, 0)
        for line in lines
    )
...
def setup_create(
    ...
    if user_count > 0 or organization_count > 0:
        raise HTTPException(
            status_code=400,
            detail="Initial company setup is already complete.",
        )
    ...
    if not clean_org or not clean_name or not clean_email:
        raise HTTPException(
            status_code=400,
            detail="All setup fields are required.",
        )
    ...
    organization = Organization(
        name=clean_org,
        slug=unique_organization_slug(db, clean_org),
        is_active=True,
    )
    db.add(organization)
    db.flush()
...
def dashboard_page(request: Request, db: Session = Depends(get_db)):
    ...
    for purchase_order in purchase_orders:
        if purchase_order.status in ("ORDERED", "PARTIALLY RECEIVED"):
            open_orders += 1
            if (
                purchase_order.expected_delivery_date is not None
                and purchase_order.expected_delivery_date.date() < today
            ):
                overdue_orders += 1
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "total_sites": db.scalar(select(func.count(Site.id))) or 0,
            "total_locations": db.scalar(select(func.count(StorageLocation.id))) or 0,
        },
    )
    ...
    return [
        {
            ...
            "default_location": location_label(db, item.default_storage_location_id),
            "physical_stock": stock_for_item(db, item.id),
            "reserved_stock": reserved_for_item(db, item.id),
            "available_stock": available_for_item(db, item.id),
            "on_order": on_order_for_item(db, item.id),
            "minimum_stock": item.min_stock,
            "supplier": item.supplier,
            "unit_cost": item.unit_cost,
            "lead_time_days": item.lead_time_days,
            "lead_time_mode": item.lead_time_mode,
        }
        for item in items
    ]
...
def inventory_page(request: Request, db: Session = Depends(get_db)):
    ...
    inventory = [
        {
            ...
            "name": item.name,
            "category": item.category,
            "location": location_label(db, item.default_storage_location_id),
            "physical_stock": stock_for_item(db, item.id),
            "reserved_stock": reserved_for_item(db, item.id),
            "available_stock": available_for_item(db, item.id),
            "on_order": on_order_for_item(db, item.id),
    ...
    return templates.TemplateResponse(
        request=request,
        name="inventory.html",
        context={"items": inventory, "storage_locations": active_locations(db)},
    )


@app.get("/inventory/{item_id}", response_class=HTMLResponse)
def inventory_item_page(
    item_id: int,
    request: Request,
    db: Session = Depends(get_db),
):
        ...
        movement_history.append(
            {
                ...
                "asset_name": asset_display_label(asset),
                "location_name": location_label(db, transaction.storage_location_id),
    ...
    location_balances = [
        {
            "id": location.id,
            "name": location_label(db, location.id),
            "physical": stock_for_item(db, item.id, location.id),
            "reserved": reserved_for_item(db, item.id, location.id),
            "available": available_for_item(db, item.id, location.id),
        }
        for location in locations
        if stock_for_item(db, item.id, location.id) != 0
        or reserved_for_item(db, item.id, location.id) != 0
        or item.default_storage_location_id == location.id
    ]
...
def create_inventory_item(
    ...
    if not clean_sku or not clean_name or not clean_category:
        raise HTTPException(status_code=400, detail="SKU, name and category are required.")
    if min_stock < 0 or lead_time_days < 0:
        raise HTTPException(status_code=400, detail="Stock and lead time values cannot be negative.")
    ...
    if db.scalar(select(Item).where(Item.sku == clean_sku)) is not None:
        raise HTTPException(status_code=400, detail="An item with this SKU already exists in this organization.")
    ...
    item = Item(
        ...
        lead_time_mode=clean_mode,
    )
    db.add(item)
    safe_commit(db, "Could not create the inventory item.")
...
def adjust_inventory_item(
    ...
    storage_location_id: int = Form(...),
    reason: str = Form("Audit count correction"),
    notes: str = Form(""),
    db: Session = Depends(get_db),
    _user=Depends(require_roles(ROLE_ADMIN, ROLE_MANAGER)),
):
...
def movements_page(request: Request, db: Session = Depends(get_db)):
    ...
    transactions = db.scalars(
        select(Transaction).order_by(Transaction.movement_date.desc())
    ).all()
...
def movement_stock_in(
    ...
    asset_id: str = Form(""),
    db: Session = Depends(get_db),
    _user=Depends(require_roles(ROLE_ADMIN, ROLE_MANAGER, ROLE_STAFF)),
):
    try:
        create_stock_in(
            db=db,
            ...
            reason=reason,
            actor=actor_name(request),
            asset_id=parse_optional_int(asset_id, "asset ID"),
            storage_location_id=storage_location_id,
...
def movement_stock_transfer(
    request: Request,
    item_id: int = Form(...),
    quantity: int = Form(...),
    from_storage_location_id: int = Form(...),
    to_storage_location_id: int = Form(...),
    reference: str = Form(""),
    notes: str = Form(""),
    db: Session = Depends(get_db),
    _user=Depends(require_roles(ROLE_ADMIN, ROLE_MANAGER, ROLE_STAFF)),
...
def receive_order_page(
    ...
    try:
        receive_purchase_order(
            ...
            quantity=quantity,
            received_by=actor_name(request),
            storage_location_id=storage_location_id,
            notes=notes or None,
        )
    except ValueError as exc:
...
def assets_page(request: Request, db: Session = Depends(get_db)):
            ...
            component_data.append(
                {
                    ...
                    "item_name": item.name if item else "Unknown Item",
                    "sku": item.sku if item else "Unknown",
                    "required": component.quantity_required,
                    "assigned": component.quantity_assigned,
                    "missing": missing,
                }
            )
        ...
        asset_data.append(
            {
                ...
                "site": site_display_label(db.get(Site, asset.site_id) if asset.site_id else None),
                ...
                "completion": round((total_assigned / total_required) * 100) if total_required else 0,
            }
        )
    ...
    return templates.TemplateResponse(
        ...
        context={
            "assets": asset_data,
            "items": items,
            "sites": active_sites(db),
            "asset_types": active_asset_types(db),
            "storage_locations": active_locations(db),
        },
    )
...
def create_asset(
    ...
    asset = Asset(
        ...
        site_id=site.id,
        serial_number=clean_serial or None,
        location=location.strip() or None,
        status=status.strip().upper() or "ACTIVE",
        notes=notes.strip() or None,
    )
    db.add(asset)
...
def add_asset_component(
    ...
    existing = db.scalar(
        select(AssetComponent).where(
            ...
            AssetComponent.item_id == item_id,
        )
    )
...
def return_asset_component(
    ...
    try:
        return_component_from_asset(
            ...
            quantity=quantity,
            returned_by=actor_name(request),
            storage_location_id=storage_location_id,
            notes=notes or None,
        )
    except ValueError as exc:
...
def requests_page(request: Request, db: Session = Depends(get_db)):
    items = db.scalars(select(Item).order_by(Item.name)).all()
    assets = db.scalars(select(Asset).order_by(Asset.code)).all()
    stock_requests = db.scalars(
        select(StockRequest).order_by(StockRequest.request_date.desc())
    ).all()
        ...
        requests_data.append(
            {
                ...
                "sku": item.sku if item else "Unknown",
                "asset_name": asset_display_label(asset),
                "location_name": location_label(db, stock_request.storage_location_id),
                ...
                "approved_by": stock_request.approved_by,
                "approved_at": stock_request.approved_at,
                "collected_at": stock_request.collected_at,
...
def approve_request_page(
    ...
    try:
        approve_request(db=db, request_id=request_id, approved_by=actor_name(request))
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return RedirectResponse("/requests", status_code=303)
...
def reject_request_page(
    ...
    try:
        reject_request(
            ...
            rejected_by=actor_name(request),
            notes=notes or None,
        )
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def collect_request_page(
    ...
    try:
        collect_request(db=db, request_id=request_id, collected_by=actor_name(request))
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return RedirectResponse("/requests", status_code=303)
...
def settings_page(
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "storage_locations": db.scalars(
                select(StorageLocation).order_by(StorageLocation.name)
            ).all(),
            "asset_types": db.scalars(select(AssetType).order_by(AssetType.name)).all(),
...
def create_storage_location(
    ...
    code: str = Form(...),
    name: str = Form(...),
    location_type: str = Form("STORAGE"),
    db: Session = Depends(get_db),
    _user=Depends(require_roles(ROLE_ADMIN)),
):
    site = db.get(Site, site_id)
    ...
    db.add(
        StorageLocation(
            ...
            code=code.strip().upper(),
            name=name.strip(),
            location_type=location_type.strip().upper() or "STORAGE",
            is_active=True,
        )
    )
...
# =========================================================
# INVENTORY INTELLIGENCE / DREI
# =========================================================


def build_intelligence_page_context(db: Session) -> dict:
...
def add_empty_drei_results(context: dict) -> None:
    context.update(
        {
...
def insights_page(request: Request, db: Session = Depends(get_db)):
    context = build_intelligence_page_context(db)
    add_empty_drei_results(context)
...
def post_drei_item_assistant(
    ...
    context = build_intelligence_page_context(db)
    add_empty_drei_results(context)
    context["assistant_question"] = question
    context["selected_item_id"] = item_id

    if not context["ai_status"]["available"]:
        context["assistant_error"] = context["ai_status"]["message"]
    else:
        try:
            result = ask_drei_about_item(db=db, item_id=item_id, question=question)
            context["assistant_answer"] = result["answer"]
            context["selected_insight"] = result["insight"]
            context["selected_anomalies"] = result["anomalies"]
        except (DreiAgentInputError, DreiAgentItemNotFoundError) as exc:
...
def post_drei_attention(
    request: Request,
    question: str = Form("What needs my attention right now?"),
    db: Session = Depends(get_db),
):
    context = build_intelligence_page_context(db)
    add_empty_drei_results(context)
    ...
    if not context["ai_status"]["available"]:
        context["attention_error"] = context["ai_status"]["message"]
    else:
        try:
            result = ask_drei_about_attention(db=db, question=question)
            context["attention_answer"] = result["answer"]
            context["attention_items"] = result["attention_items"]
        except DreiAgentInputError as exc:
            context["attention_error"] = str(exc)
        except LocalAIError as exc:
            context["attention_error"] = str(exc)
...
def post_drei_overdue_orders(
    request: Request,
    question: str = Form("Which purchase orders are overdue?"),
    db: Session = Depends(get_db),
):
    context = build_intelligence_page_context(db)
    add_empty_drei_results(context)
    ...
    if not context["ai_status"]["available"]:
        context["overdue_error"] = context["ai_status"]["message"]
    else:
        try:
            result = ask_drei_about_overdue_orders(db=db, question=question)
            context["overdue_answer"] = result["answer"]
            context["overdue_orders"] = result["overdue_orders"]
        except DreiAgentInputError as exc:
            context["overdue_error"] = str(exc)
        except LocalAIError as exc:
            context["overdue_error"] = str(exc)
